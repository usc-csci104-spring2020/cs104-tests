    ...
    if(cmd=="create"){
    	...
    	if(ss.fail()){
    		...
    		if(ss.fail()){
    			...
    			if(i!=tag_map.end() && i->second<=(int)commits.size()){
    				checkout(s_arg1);
    			}else{
    				throw std::invalid_argument(INVALID_OPTION);
    			}
    	...
    	if(ss.fail()){
    		...
    		diff(current_commit,total_commit);
    	}
    }
    else if(cmd=="quit"){
    	quit=true;
...
void GitInt::diff(CommitIdx to) const{
    if(valid_commit(to) && to>=0){
        if(total_commit==0){
            ...
            map<string,int> diff=buildState(current_commit,to);
            display_helper(diff); 
            //if(diff.empty()) DEBUG_MSG("DSJHJFAAAAAAAAAAAAAAAAAA");   
        }
    }else{
        throw std::invalid_argument(INVALID_OPTION);
    }
...
void GitInt::diff(CommitIdx from, CommitIdx to) const{
    if(from>=to && from>=0 && to>=0){
        if(total_commit==0){
            ...
            map<string,int> diff=buildState(from,to);
            display_helper(diff);    
        }    
    }else{
        throw std::invalid_argument(INVALID_OPTION);
